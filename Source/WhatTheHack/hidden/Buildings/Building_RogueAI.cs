...
public class Building_RogueAI : Building
{
    ...
    private readonly float moodDrainCtrlMech = 0.075f;
    private readonly float moodDrainCtrlTur = 0.075f;
    private readonly float moodDrainDamage = 2.0f;
    private readonly float moodDrainForceTalkGibberish = 5.0f;
    private readonly float moodDrainHack = 0.3f;
    private readonly float moodDrainNoPower = 0.2f;
    private readonly List<ActionItem> queuedActions = new List<ActionItem>();
    private int abilityWarmUpTicks;
    ...
    private int currentAbilityTicksTotal;
    public bool goingRogue;
    public List<Pawn> hackedMechs = new List<Pawn>();
    private bool isConscious;
    public bool managingPowerNetwork;
    ...
    public bool WarmingUpAbility => abilityWarmUpTicks > 0;


    public CompRefuelable RefuelableComp => GetComp<CompRefuelable>();

    private CompDataLevel DataLevelComp => GetComp<CompDataLevel>();

    public CompPowerPlant_RogueAI PowerPlantComp => PowerComp as CompPowerPlant_RogueAI;

    private CompOverlay OverlayComp => GetComp<CompOverlay>();
    ...
    public Mood CurMoodCategory
    {
        get
        {
            ...
            if (!(RefuelableComp.FuelPercentOfMax < 0.45f))
            {
                return Mood.Happy;
            }
    ...
    public bool CurrentlyDrainingMood =>
        hackedMechs.Count > 0 || controlledMechs.Count > 0 || controlledTurrets.Count > 0;
    ...
    public override void Destroy(DestroyMode mode = DestroyMode.Vanish)
    {
        CancelLinks();
        StopGoingRogue();
        foreach (var beacon in Map.listerBuildings.allBuildingsColonist.Where(b => b is Building_MechanoidBeacon)
    ...
    public override void TickRare()
    {
        ...
        var mapData = store.GetExtendedDataFor(Map);
        mapData.rogueAI = this;
    }

    private void MaybeTalkGibberish()
    {
        var textChance = goingRogue ? 1.0f : 0.02f;
    ...
    private void TalkGibberish()
    {
        ...
        var randomColonistName = Map.mapPawns.FreeColonists.RandomElement().Name.ToStringShort;
        string signalFromEarth =
            ("WTH_RogueAI_SignalFromEarth_" + Rand.RangeInclusive(0, NUMTEXTSSIGNALFROMEARTH - 1)).Translate();
        string signalFromEarthResponse =
            ("WTH_RogueAI_SignalFromEarth_Response_" + Rand.RangeInclusive(0, NUMTEXTSSIGNALFROMEARTHRESPONSE - 1))
            .Translate();
        string text;
        ...
        OutputText(text.Translate(randomColonistName, signalFromEarth, signalFromEarthResponse));
    }
    ...
    private void OutputText(string text, bool force = false)
    {
        ...
        Utilities.ThrowStaticText(DrawPos + new Vector3(0, 0, z), Map, text, color, TEXTDURATION);
    ...
    private void MaybeGoRogue()
    {
        ...
        var goRogueChance = 1 / (RefuelableComp.Fuel * 2.5f);
        if (!Rand.Chance(goRogueChance))
        {
            return;
        }

        Find.LetterStack.ReceiveLetter("WTH_Message_GoRogue_Label".Translate(),
            "WTH_Message_GoRogue_Description".Translate(), LetterDefOf.ThreatBig, new GlobalTargetInfo(this));
        GoRogue();
    }
    ...
    {
        foreach (var unused in controlledMechs)
        {
            DrainMood(moodDrainCtrlMech);
        }

        foreach (var unused in controlledTurrets)
        {
            DrainMood(moodDrainCtrlTur);
        }

        foreach (var unused in hackedMechs)
        {
    ...
    public override IEnumerable<Gizmo> GetGizmos()
    {
        ...
        foreach (var mech in controlledMechs)
        {
        ...
        foreach (var mech in hackedMechs)
        {
    ...
    {
        foreach (var actionItem in queuedActions)
        {
            actionItem.Tick();
        }

        queuedActions.RemoveAll(i => i.shouldClean);
    }

    private void DrawWarmup()
    {
        var height = 0.5f;
        var width = 0.25f * (1 - (abilityWarmUpTicks / (float)currentAbilityTicksTotal));
        //GenDraw.DrawCooldownCircle(this.Position.ToVector3Shifted() + new Vector3(0, 3f, 0), width);

        var s = new Vector3(width, 10f, height);
        var matrix = default(Matrix4x4);
        matrix.SetTRS(Position.ToVector3Shifted() + new Vector3(0, 3f, 0), Quaternion.identity, s);
        Graphics.DrawMesh(MeshPool.plane20, matrix,
            SolidColorMaterials.SimpleSolidColorMaterial(new Color(1f, 1f, 1f, 0.5f)), 0);
    }
    ...
    private Gizmo GetTalkGibberishGizmo()
    {
        ...
        {
            defaultLabel = "WTH_Gizmo_TalkGibberish_Label".Translate(),
            defaultDesc = "WTH_Gizmo_TalkGibberish_Description".Translate(moodDrainForceTalkGibberish),
            icon = ContentFinder<Texture2D>.Get("UI/RogueAI_TalkGibberish"),
            disabled = GlobalShouldDisable(out var reason),
    ...
    private Gizmo GetManagePowerNetworkGizmo()
    {
        ...
        {
            defaultLabel = "WTH_Gizmo_ManagePowerNetwork_Label".Translate(),
            defaultDesc = "WTH_Gizmo_ManagePowerNetwork_Description".Translate(),
            icon = ContentFinder<Texture2D>.Get("UI/RogueAI_Manage_Network"),
            disabled = GlobalShouldDisable(out var reason),
            disabledReason = reason
        };
        ...
        command.isActive = () => managingPowerNetwork;
    ...
    {
        var command = new Command_Action_Highlight
        {
            defaultLabel = mech.Name.ToStringShort,
            defaultDesc = "WTH_Gizmo_ControlMechanoidCancel_Description".Translate(),
            parent = this,
            thing = mech
        };

        var iconFound = Base.Instance.cancelControlMechTextures.TryGetValue(mech.def.defName, out var icon);
        if (iconFound)
        {
            command.icon = icon;
        }

        command.action = delegate { CancelControlMechanoid(mech); };
        return command;
    }
    ...
    private Gizmo GetControlMechanoidActivateGizmo()
    {
        ...
        {
            defaultLabel = "WTH_Gizmo_ControlMechanoidActivate_Label".Translate(),
            defaultDesc = "WTH_Gizmo_ControlMechanoidActivate_Description".Translate(MAXCONTROLLABLEMECHS),
            targetingParams = GetTargetingParametersForControlling(),
            hotKey = KeyBindingDefOf.Misc5,
            icon = ContentFinder<Texture2D>.Get("UI/RogueAI_Control")
        };

        var shouldDisable = GlobalShouldDisable(out var reason);
        ...
        {
            if (target is { HasThing: true, Thing: Pawn mech })
            {
    ...
    private Gizmo GetHackingCancelGizmo(Pawn mech, int index)
    {
        var command = new Command_Action_Highlight
        {
            defaultLabel = "WTH_Gizmo_HackingCancel_Label".Translate() + " " + index,
            defaultDesc = "WTH_Gizmo_HackingCancel_Description".Translate(),
            parent = this,
            thing = mech
        };
        ...
        command.action = delegate { CancelHacking(mech); };
        return command;
    }
    ...
    private Gizmo GetHackingAvtivateGizmo()
    {
        var command = new Command_Target
        {
            defaultLabel = "WTH_Gizmo_HackingAvtivate_Label".Translate(),
            defaultDesc = "WTH_Gizmo_HackingAvtivate_Description".Translate(MAXHACKABLE),
            targetingParams = GetTargetingParametersForHacking(),
            hotKey = KeyBindingDefOf.Misc5,
            icon = ContentFinder<Texture2D>.Get("UI/RogueAI_Hack")
        };
        var shouldDisable = GlobalShouldDisable(out var reason);
        command.disabled = shouldDisable;
        command.disabledReason = reason;
    ...
    private static TargetingParameters GetTargetingParametersForHacking()
    {
        return new TargetingParameters
        {
            ...
            validator = delegate(TargetInfo targ)
            {
                ...
                var pawn = targ.Thing as Pawn;
                return pawn is { Downed: false } && pawn.IsMechanoid() && pawn.Faction != Faction.OfPlayer;
            }
        };
    }
    ...
    private Gizmo GetControlTurretCancelGizmo(Building_TurretGun turret, int index)
    {
        var command = new Command_Action_Highlight
        {
            defaultLabel = "WTH_Gizmo_ControlTurretCancel_Label".Translate() + index,
            defaultDesc = "WTH_Gizmo_ControlTurretCancel_Description".Translate(),
            parent = this,
            thing = turret
        };

        var iconFound = Base.Instance.cancelControlTurretTextures.TryGetValue(turret.def.defName, out var icon);
        if (iconFound)
        {
            command.icon = icon;
        }

        command.action = delegate { CancelControlTurret(turret); };
        return command;
    }
    ...
    private Gizmo GetControlTurretAvtivateGizmo()
    {
        var command = new Command_Target
        {
            defaultLabel = "WTH_Gizmo_ControlTurretActivate_Label".Translate(),
            defaultDesc = "WTH_Gizmo_ControlTurretActivate_Description".Translate(MAXCONTROLLABLETURRETS),
            targetingParams = GetTargetingParametersForControlTurret(),
            icon = ContentFinder<Texture2D>.Get("UI/RogueAI_Control_Turret")
        };
        var shouldDisable = GlobalShouldDisable(out var reason);
        command.disabled = shouldDisable;
        command.disabledReason = reason;
        ...
        command.action = delegate(LocalTargetInfo target)
        {
            if (target is { HasThing: true, Thing: Building_TurretGun turret })
            {
    ...
    private static TargetingParameters GetTargetingParametersForControlTurret()
    {
        return new TargetingParameters
        {
            ...
            validator = delegate(TargetInfo targ)
            {
                ...
                return building is Building_TurretGun turret
                       && turret.Faction == Faction.OfPlayer
                       && turret.mannableComp == null;
            }
        };
    }
    ...
    public void GoRogue()
    {
        ...
        goingRogue = true;
        SetFaction(Faction.OfMechanoids);
        UpdateGlower();
        CancelLinks();
        QueueGoRogueActions();
    }
    ...
    private void QueueGoRogueActions()
    {
        ...
        void CauseZzzttAction()
        {
            GoRogue_CauseZzztts();
        }

        void HackTurretsAction()
        {
            GoRogue_HackTurrets();
        }

        void HackMechsAction()
        {
            GoRogue_HackMechs();
        }
        ...
        var duration = Rand.Range(10000, 15000);
        var actionInterval =
            (duration + 4000) /
            (possibleActions.Count + 1); //add 4000 to duration so not every action can happen during the duration. 
        var actionDelay = 10;
        while (!possibleActions.NullOrEmpty())
        {
            var action = possibleActions.RandomElement();
    ...
    public void StopGoingRogue()
    {
        ...
        Messages.Message("WTH_Message_RogueAI_StopGoingRogue".Translate(), new GlobalTargetInfo(Position, Map),
            MessageTypeDefOf.PositiveEvent);
    }

    public void UpdateGlower()
    {
        var glowerComp = GetComp<CompGlower>();
        var mood = CurMoodCategory;
    ...
    {
        var shouldHack = Map.mapPawns.SpawnedPawnsInFaction(Faction.OfPlayer).Where(p => p.IsHacked() && !p.Downed)
            .ToList();
        var nShouldHack = Rand.Range(1, 4);
        var nHacked = 0;
        while (nShouldHack > 0 && shouldHack.Count > 0)
        {
            var mech = shouldHack.RandomElement();
            mech.RevertToFaction(Faction.OfMechanoids);
            mech.jobs.EndCurrentJob(JobCondition.InterruptForced);
            if (mech.GetLord() == null || mech.GetLord().LordJob == null)
            {
                LordMaker.MakeNewLord(Faction.OfMechanoids, new LordJob_AssaultColony(Faction.OfMechanoids), mech.Map,
                    new List<Pawn> { mech });
            }
        ...
        if (nHacked > 0)
        {
            Find.LetterStack.ReceiveLetter("WTH_Message_GoRogue_HackMechs_Label".Translate(),
                "WTH_Message_GoRogue_HackMechs_Description".Translate(), LetterDefOf.ThreatBig,
                new LookTargets(rogueMechs));
        }
    }

    private void GoRogue_HackTurrets()
    {
        bool IsSuitableTurret(Thing t)
        {
            return t is Building_TurretGun turret && turret.Faction == Faction.OfPlayer && turret.mannableComp == null;
        }

        //Traverse.Create(turret).Field("mannableComp").GetValue<CompMannable>() == null;
        var turrets = Map.spawnedThings.Where(IsSuitableTurret).Cast<Building_TurretGun>().ToList();
        var nShouldHack = Rand.Range(1, 3);
        var nHacked = 0;
        while (nShouldHack > 0 && turrets.Count > 0)
        {
            var turret = turrets.RandomElement();
        ...
        if (nHacked > 0)
        {
            Find.LetterStack.ReceiveLetter("WTH_Message_GoRogue_HackTurrets_Label".Translate(),
                "WTH_Message_GoRogue_HackTurrets_Description".Translate(), LetterDefOf.ThreatBig,
                new LookTargets(rogueTurrets.Cast<Thing>()));
        }
    }
    ...
    private void GoRogue_CauseZzztts()
    {
        ...
        var potentialTargets = ShortCircuitUtility.GetShortCircuitablePowerConduits(Map);
        var numZzztts = Rand.Range(3, 5);
        var rareTicksUntilAction = 0;
        for (var i = 0; i < numZzztts; i++)
        {
            if (!potentialTargets.TryRandomElement(out var potentialTarget))
            {
                continue;
            }

            queuedActions.Add(new ActionItem(
                delegate { ShortCircuitUtility.DoShortCircuit(potentialTarget); },
        ...
        queuedActions.Add(new ActionItem(
            delegate { PowerPlantComp.overcharging = false; },
            rareTicksUntilAction)
        );

        Find.LetterStack.ReceiveLetter("WTH_Message_GoRogue_CauseZzztts_Label".Translate(),
            "WTH_Message_GoRogue_CauseZzztts_Description".Translate(), LetterDefOf.ThreatBig,
    ...
    private class Command_Action_Highlight : Command_Action
    {
        ...
        public override void GizmoUpdateOnMouseover()
        {
            ...
            GenDraw.DrawLineBetween(parent.Position.ToVector3Shifted(), thing.Position.ToVector3Shifted(),
                SimpleColor.White);
        }
    }
    ...
    private class ActionItem
    {
        ...
        {
            this.action = action;
            ticksUntilAction = tickUntilAction;
        }
